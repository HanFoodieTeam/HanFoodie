...
function isHetMonByDate(hetMon?: Date | string | null): boolean {
  if (!hetMon) return false;
  ...
  return hetMonDate <= today; // <= hôm nay là HẾT
}
...
const danhSachDaXuLy: typeof danh_sach_san_pham = [];

for (const sp of danh_sach_san_pham) {
  if (!sp.id_bien_the) continue;
  ...
  const giaGoc = bienThe.getDataValue("san_pham")?.gia_goc ?? 0;
  const giaThemBienThe = bienThe.getDataValue("gia_them") ?? 0;

  let monThemHopLe: { ten: string; gia_them?: number }[] = [];
  let giaMonThem = 0;

  if (sp.json_mon_them) {
    let arr: { ten: string; gia_them?: number }[] = [];
    try {
      arr = JSON.parse(sp.json_mon_them);
    } catch {}

    if (arr.length > 0) {
      const tenList = arr.map((m) => m.ten);

      const monThemDB = await MonThemModel.findAll({
        where: { ten: tenList, trang_thai: true },
      });

      monThemHopLe = monThemDB
        .filter((m) => !isHetMonByDate(m.getDataValue("het_mon")))
        .map((m) => ({
          ten: m.getDataValue("ten"),
          gia_them: m.getDataValue("gia_them"),
        }));

      giaMonThem = monThemHopLe.reduce(
        (s, m) => s + (m.gia_them ?? 0),
        0
      );
    }
  ...
  const donGia = giaGoc + giaThemBienThe + giaMonThem;
  const thanhTien = donGia * sp.so_luong;

  tong_tien_hang += thanhTien;

  danhSachDaXuLy.push({
    ...sp,
    don_gia: donGia,
    json_mon_them:
      monThemHopLe.length > 0 ? JSON.stringify(monThemHopLe) : null,
  });
}
        ...
        //  Lưu chi tiết đơn hàng
        for (const sp of danhSachDaXuLy) {
            const thanh_tien = sp.don_gia * sp.so_luong;
