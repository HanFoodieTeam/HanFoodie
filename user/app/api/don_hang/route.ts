...
// // type ChiTietDonHangWithRelations = {
// //   id: number;
// //   so_luong: number;
// //   json_tuy_chon?: string | null;
// //   json_mon_them?: string | null;
...
// // type DonHangWithCT = IDonHang & {
// //   chi_tiet_don_hang: ChiTietDonHangWithRelations[];
// // };
...
// //     const where: Record<string, unknown> = { id_nguoi_dung: nguoiDung.id };
// //     if (trang_thai && trang_thai !== "tat_ca") where.trang_thai = trang_thai;

// //     const { rows, count } = await DonHangModel.findAndCountAll({
// //       where,
// //       include: [
// //         {
// //           model: ChiTietDonHangModel,
...
// //     // --------------------------
// //     // ĐỔI DỮ LIỆU SANG ĐÚNG TYPE
// //     // --------------------------
// //     const donHangDaXuLy = rows.map((dh) => {
// //       const dataValues = dh.toJSON() as DonHangWithCT;

// //       const danh_sach_san_pham = dataValues.chi_tiet_don_hang.map((ct) => ({
// //         id: ct.id,
// //         ten: ct.bien_the?.san_pham?.ten ?? "Không rõ tên",
// //         hinh: ct.bien_the?.san_pham?.hinh ?? "/noimg.png",
// //         gia: ct.bien_the?.san_pham?.gia_goc ?? 0,
...
// //         gia_them: ct.bien_the?.gia_them,
// //         id_bien_the: ct.id_bien_the,
// //         json_tuy_chon: safeParseJSON(ct.json_tuy_chon, {}),
// //         json_mon_them: safeParseJSON(ct.json_mon_them, []),
// //       }));
...
// /* Hàm chuẩn hóa URL Cloudinary */
// function getCloudinaryUrl(hinh: string | null | undefined): string {
//   if (!hinh || hinh.trim() === "") {
//     return "/noimg.png"; // fallback
//   }

//   // Nếu đã là Cloudinary URL → dùng luôn
//   if (hinh.startsWith("http")) return hinh;

//   // Nếu lưu file cũ (vd: /uploads/sp1.jpg) → tự convert sang Cloudinary folder mặc định
//   return `https://res.cloudinary.com/dqqicnjrm/image/upload/${hinh}`;
// }
...
//         id: ct.id,
//         ten: ct.bien_the?.san_pham?.ten ?? "Không rõ tên",
//         hinh: getCloudinaryUrl(ct.bien_the?.san_pham?.hinh ?? null), // ✔ Cloudinary
//         gia: ct.bien_the?.san_pham?.gia_goc ?? 0,
//         so_luong: ct.so_luong,
//         ten_bien_the: ct.bien_the?.ten,
//         gia_them: ct.bien_the?.gia_them,
...
/* Chuẩn hóa URL ảnh */
function normalizeImage(hinh?: string | null): string {
  if (!hinh || hinh.trim() === "") return "/noimg.png";

  // Nếu đã là URL → giữ nguyên (Cloudinary / Google / Facebook / Zalo)
  if (hinh.startsWith("http")) return hinh;

  // Nếu là ảnh local (uploads/...) → convert sang Cloudinary
  return `https://res.cloudinary.com/dqqicnjrm/image/upload/${hinh}`;
}
...
export async function GET(req: NextRequest) {
  try {
    ...
    const donHangDaXuLy = rows.map((dh) => {
      const data = dh.toJSON() as DonHangWithCT;

      const danh_sach_san_pham = data.chi_tiet_don_hang.map((ct) => ({
        id: ct.id,
        ten: ct.bien_the?.san_pham?.ten ?? "Không rõ tên",
        hinh: normalizeImage(ct.bien_the?.san_pham?.hinh ?? null),
        gia: ct.bien_the?.san_pham?.gia_goc ?? 0,
        so_luong: ct.so_luong,
        ten_bien_the: ct.bien_the?.ten,
        gia_them: ct.bien_the?.gia_them ?? 0,
        id_bien_the: ct.id_bien_the ?? null,
        json_tuy_chon: safeParseJSON(ct.json_tuy_chon, {}),
        json_mon_them: safeParseJSON(ct.json_mon_them, []),
      }));
    ...
    console.error("🔥 Lỗi GET /api/don_hang:", err);
    return NextResponse.json({ thong_bao: "Lỗi server" }, { status: 500 });
  }
}
