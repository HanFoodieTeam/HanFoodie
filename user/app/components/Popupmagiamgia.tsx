...
export default function PopupMaGiamGia({ open, onClose, onSelect }: Props) {
          ...
          <div className="p-4 space-y-3">
            {loading ? (
              ...
              danhSach.map((item) => (
                <div
                  key={item.id}
                  className={`flex items-center gap-4 border rounded-xl p-3 shadow-sm hover:shadow-md cursor-pointer transition ${chon === item.id ? "border-[#e8594f] bg-[#fff5f4]" : "border-gray-200"
                    }`}
                  onClick={() => setChon(item.id)}>
                  {/* Biểu tượng */}
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-600 mt-1">
                      ...
                      {item.gia_tri_giam_toi_da &&
                        ` (tối đa ${item.gia_tri_giam_toi_da.toLocaleString("vi-VN")}đ)`}
                    </p>
