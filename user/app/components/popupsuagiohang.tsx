...
selectedMonThem.forEach((id) => {
  ...
  if (isHetMonByDate(m.het_mon)) return;
  ...
  const removedMonThem = useMemo(() => {
    if (!mac_dinh.json_mon_them) return [];

    return mac_dinh.json_mon_them
      .map((m) => {
        const full = mon_them.find((x) => x.id === m.id);
        if (!full) return null;

        return isHetMonByDate(full.het_mon) ? m : null;
      })
      .filter(Boolean) as { id: number; ten: string; gia_them?: number | null }[];
  }, [mac_dinh.json_mon_them, mon_them]);

  const wasSelectedBefore = (id: number) => {
    return mac_dinh.json_mon_them?.some((m) => m.id === id);
  };
    ...
    <AnimatePresence>
      <motion.div
        ...
        <motion.div
            ...
            {mon_them.length > 0 && (
              <div>
                ...
                <div className="space-y-2">
                  ...
                  {mon_them.map((m) => {
                    const isHetMon = isHetMonByDate(m.het_mon);
                    const checked = selectedMonThem.includes(m.id!);
                    const selectedBefore = wasSelectedBefore(m.id!);
          ...
          ${isHetMon ? "opacity-50 cursor-not-allowed" : "hover:bg-gray-50 cursor-pointer"}
        `}>
                          <div className="flex items-center gap-3">
                            <input
                              type="checkbox"
                              ...
                              disabled={isHetMon}
                              onChange={() => toggleMonThem(m.id!)}
                              className="w-5 h-5 accent-[#e8594f]"
                            />
                        ...
                        {isHetMon && selectedBefore && (
                          <div className="ml-8 mb-1 text-sm text-red-600 flex items-start gap-1">
                            <span>
                              Món <b>{m.ten}</b> đã hết và được tự động bỏ khỏi đơn hàng.
                            </span>
                          </div>
                        )}
                      </div>
