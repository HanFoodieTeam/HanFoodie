...
export default function RegisterForm({ onClose, onRegisterSuccess }: RegisterFormProps) {
  const [ho_ten, setHoTen] = useState("");
  const [email, setEmail] = useState("");
  const [sdt, setSdt] = useState<number | undefined>();
  const [mat_khau, setMatKhau] = useState("");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    onRegisterSuccess(data.nguoi_dung);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      <input type="text" placeholder="Họ tên" value={ho_ten} onChange={(e) => setHoTen(e.target.value)} className="border w-full p-2 rounded" disabled={dangThanhCong} />
      <input type="email" placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} className="border w-full p-2 rounded" disabled={dangThanhCong} />
      <input type="number" placeholder="Số điện thoại" value={sdt ?? ""} onChange={(e) => setSdt(Number(e.target.value))} className="border w-full p-2 rounded" disabled={dangThanhCong} />
      <input type="password" placeholder="Mật khẩu" value={mat_khau} onChange={(e) => setMatKhau(e.target.value)} className="border w-full p-2 rounded" disabled={dangThanhCong} />
      <input type="password" placeholder="Gõ lại mật khẩu" value={go_lai_mat_khau} onChange={(e) => setGoLaiMatKhau(e.target.value)} className="border w-full p-2 rounded" disabled={dangThanhCong} />

      {thongBao && (
        <p className={`text-center text-sm ${dangThanhCong ? "text-green-600 font-medium" : "text-red-500"}`}>{thongBao}</p>
      )}

      <button
        type="submit"
        disabled={dangThanhCong}
        className={`w-full p-2 rounded text-white transition-all ${dangThanhCong ? "bg-green-600 cursor-not-allowed" : "bg-[#6A0A0A] hover:opacity-90"}`}
      >
        {dangThanhCong ? "Đăng ký thành công" : "Đăng ký"}
      </button>
    </form>
