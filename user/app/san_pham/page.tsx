...
function SanPhamContent() {
  ...
  useEffect(() => {
    ...
    let targetDanhMuc = dsDanhMuc.find(
      (dm) =>
  ...
  const toggleFavorite = async (id_san_pham: number) => {
    ...
    try {
      if (favorites.includes(id_san_pham)) {
  ...
  return (
    <main className="bg-gray-50 min-h-screen">
      ...
      <div className="py-0">
        {dsDanhMuc.map((dm) => (
          <section key={dm.id} id={`danh-muc-${dm.slug}`} className="scroll-mt-[120px]">
                ...
                return (
                  ...
                  >
                    ...
                    <Link href={`/chi_tiet/${sp.id}`} className="block p-4">
                      <h3 className="font-medium line-clamp-1">{sp.ten}</h3>
                      <div
