...
export default function DanhMucList() {
  ...
  const confirmToggle = async () => {
    ...
    try {
      ...
      setData((prev) =>
        prev.map((dm) =>
          dm.id === id ? { ...dm, an_hien: newState } : dm
        )
      );
    } catch {
    ...
    <div className="p-3 md:p-6">
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-3 mb-4">
        ...
        <div className="flex flex-col sm:flex-row gap-2 items-stretch sm:items-center">
          <div className="relative flex items-center border border-gray-400 rounded-lg px-3 py-2 bg-white">
            <input
              type="text"
              placeholder="Tìm theo tên..."
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="outline-none w-full sm:w-64 text-sm"
            />
            {search && (
              <button
                onClick={() => {
      ...
      <div className="overflow-x-auto bg-white rounded-2xl shadow-lg border border-gray-200">
        <table className="min-w-[900px] md:min-w-full text-sm text-center border-collapse">
          <thead className="bg-gray-300 text-gray-700 uppercase text-xs md:text-sm">
            <tr>
              ...
              <th className="px-3 py-2">Số SP</th>
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={9} className="py-6 text-gray-400 italic">
                  Không có dữ liệu
                </td>
              </tr>
            ) : (
              data.map((dm, i) => (
                ...
                >
                  ...
                  <td
                    className="px-3 py-2 cursor-pointer text-xl"
      ...
      {confirmItem && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-3">
          <div className="bg-white rounded-xl p-5 md:p-6 shadow-lg w-full max-w-sm text-center">
