...
// Cấu trúc dữ liệu loại bài viết
...
interface ILoaiBaiVietResponse {
  success: boolean;
  data: ILoaiBaiViet[];
  totalPages: number;
  totalItems: number;
...
function LoaiBaiVietListContent() {
  const router = useRouter();
  const searchParams = useSearchParams();

  const [page, setPage] = useState<number>(() => Number(searchParams.get("page")) || 1);
  const [searchQuery, setSearchQuery] = useState<string>(() => searchParams.get("search") || "");
  const [data, setData] = useState<ILoaiBaiViet[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [search, setSearch] = useState<string>(searchQuery);
  const [totalPages, setTotalPages] = useState<number>(1);
  const [confirmItem, setConfirmItem] = useState<ILoaiBaiViet | null>(null);
  ...
  useEffect(() => {
    ...
    router.replace(`/loai_bai_viet?${params.toString()}`);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const res = await fetch(`/api/loai_bai_viet?${qs.toString()}`);
      const json: ILoaiBaiVietResponse = await res.json();
      if (json.success) {
        setData(json.data);
      ...
      console.error("Lỗi khi tải danh sách loại bài viết:", err);
      setData([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleToggleClick = (item: ILoaiBaiViet) => setConfirmItem(item);
  ...
  return (
    <div>
      ...
      <div className="flex justify-between items-center mb-4 flex-wrap gap-2">
        <h1 className="text-2xl font-bold text-gray-800">Quản lý Loại Bài Viết</h1>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 shadow-lg w-[380px] text-center">
            ...
            <p className="mb-5 text-lg">
              ...
              loại <span className="font-semibold">{confirmItem.ten_loai}</span> không?
            </p>
...
export default function LoaiBaiVietList() {
  return (
    <Suspense fallback={<div className="p-4 text-lg">Đang tải...</div>}>
      <LoaiBaiVietListContent />
    </Suspense>
  );
}
