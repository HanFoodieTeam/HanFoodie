...
//   const handleSearch = () => updateQuery({ search: search.trim(), page: "1" });

//   //  Khi bấm vào icon trạng thái -> mở modal xác nhận
...
//   if (loading) return <div className="p-4 text-lg">Đang tải dữ liệu...</div>;
...
//           <input type="text" placeholder="Tìm theo tên món..." value={search}
...
//             ) : (
//               data.map((item) => (
//                 <tr key={item.id} className="border-t hover:bg-gray-100 transition-all text-b" >
...
//                     onClick={() => handleToggleClick(item)} title="Bấm để đổi trạng thái">
//                     {item.trang_thai ? "✅" : "❌"}
//                   </td>

//                   {/*  Sửa / Xóa */}
//                   <td className="px-5 py-4 text-center">
//                     <Link href={`/mon_them/${item.id}`}
            ...
            {data.length === 0 ? (
              ...
              data.map((item) => (
                <tr key={item.id} className="border-t hover:bg-gray-100 transition-all" >
                  ...
                  {/* Trạng thái có thể bấm đổi */}
                  <td className="px-5 py-4 text-center cursor-pointer select-none text-2xl" onClick={() => handleToggleClick(item)} title="Bấm để đổi trạng thái">
                    {item.trang_thai ? "✅" : "❌"}
                  </td>
                  ...
                  <td className="px-5 py-4 text-center">
                    ...
                    {" | "}
                    <button onClick={() => handleDelete(item)} className="text-red-600 hover:text-red-800 font-semibold">
                      Xóa
                    </button>
                  </td>
                </tr>
