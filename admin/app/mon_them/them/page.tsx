...
export default function ThemMonThem() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const res = await fetch("/api/mon_them", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(form),
    });
    if (res.ok) {
      alert(" Thêm món thêm thành công!");
  ...
  return (
    <div className="p-2 bg-white rounded-xl shadow-md  mx-auto">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {/* Cột 1 */}
        <div>
          <label className="block mb-1 font-medium text-gray-700">Tên món thêm</label>
          <input name="ten" value={form.ten} onChange={handleChange} placeholder="VD: Trứng chiên, Phô mai, ..."
            className="border border-gray-300 p-2 rounded w-full focus:outline-none focus:ring-2 focus:ring-blue-400" />
        </div>

        <div>
          <label className="block mb-1 font-medium text-gray-700">Giá thêm (VNĐ)</label>
