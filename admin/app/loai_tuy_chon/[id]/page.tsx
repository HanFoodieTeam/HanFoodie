...
export default function SuaLoaiTuChon() {
  const router = useRouter();
  const { id } = useParams();

  const [form, setForm] = useState<ILoaiTuChon>({
    id: 0,
    ten: "",
    thu_tu: 0,
    an_hien: true,
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const json: { success: boolean; data: ILoaiTuChon } = await res.json();
  ...
  return (
    <div className="p-6 bg-white rounded-xl shadow-md max-w-4xl mx-auto">
      ...
      >
        {/* Tên loại */}
        <div>
          <label className="font-medium">Tên loại</label>
          <input
            name="ten"
            value={form.ten}
        ...
        <div className="md:col-span-2 flex justify-end mt-4">
          ...
          >
            {loading ? "Đang lưu..." : "Cập nhật loại"}
          </button>
        </div>
      </form>
    </div>
